        ...
        for _ in 0..self.total_vertices - 1 {
            for edge in &self.edges {
                ...
                if distances[edge.start_node] != f64::INFINITY
                    && distances[edge.start_node] + edge.log_conversion_value
                        < distances[edge.end_node]
                {
                    distances[edge.end_node] =
                        distances[edge.start_node] + edge.log_conversion_value;
                    trace!(
                        "New Edge Node Distance Calculated {}",
                        distances[edge.end_node]
                    );
    ...
    fn test_creation_of_graph() {
        ...
        let test_input = vec![
            IndexedGraphEdge {
                ...
                log_conversion_value: -1.0 * (57157.97132034f64).ln(),
            },
            IndexedGraphEdge {
                start_node: currency_map["EUR"],
